...
public class MainActivity extends AppCompatActivity {
    ...
    void buildUi(){
        ...
        TextView help=new TextView(this); help.setText("Keep this app open while connecting. Screen sharing always uses Android's system permission dialog. Touch control requires you to explicitly enable PhoneBridge in Accessibility settings."); help.setPadding(0,20,0,0); root.addView(help);
        setContentView(root);
    }
    ...
    @Override protected void onActivityResult(int req,int res,Intent data){
        ...
        if(req==REQUEST_TREE&&res==RESULT_OK&&data!=null){ treeUri=data.getData(); try{getContentResolver().takePersistableUriPermission(treeUri,Intent.FLAG_GRANT_READ_URI_PERMISSION|Intent.FLAG_GRANT_WRITE_URI_PERMISSION);}catch(Exception ignored){} storageView.setText("Storage access: ready\n"+treeUri); }
        if(req==REQUEST_SCREEN){ Client c=pendingScreenClient; pendingScreenClient=null; if(res==RESULT_OK&&data!=null&&c!=null){ MediaProjectionManager m=(MediaProjectionManager)getSystemService(MEDIA_PROJECTION_SERVICE); mediaProjection=m.getMediaProjection(res,data); startScreen(c); } else if(c!=null){ try{c.writeJson(new JSONObject().put("ok",false).put("error","Screen capture permission was denied"));}catch(Exception ignored){} } }
    }
    void startServices(){ server=new Server(); server.start(); announce=()->{announce();handler.postDelayed(announce,2000);}; handler.post(announce); }
    void announce(){ new Thread(()->{ try{DatagramSocket s=new DatagramSocket();s.setBroadcast(true);String name=Build.MODEL.replace(" ","_");String msg="PHONEBRIDGE/1 DISCOVER "+deviceId()+" "+name+" "+server.port;byte[] b=msg.getBytes(StandardCharsets.UTF_8);s.send(new DatagramPacket(b,b.length,InetAddress.getByName("255.255.255.255"),DISCOVERY_PORT));s.close();}catch(Exception ignored){} }).start(); }
    ...
    void startScreen(Client c){
        ...
        imageReader.setOnImageAvailableListener(r->{Image image=null;try{image=r.acquireLatestImage();if(image==null||screenClient==null)return;Image.Plane p=image.getPlanes()[0];int pixel=p.getPixelStride(),row=p.getRowStride();ByteBuffer buf=p.getBuffer();int padded=Math.max(captureWidth,row/pixel);Bitmap full=Bitmap.createBitmap(padded,captureHeight,Bitmap.Config.ARGB_8888);full.copyPixelsFromBuffer(buf);Bitmap bmp=(padded==captureWidth)?full:Bitmap.createBitmap(full,0,0,captureWidth,captureHeight);ByteArrayOutputStream baos=new ByteArrayOutputStream();bmp.compress(Bitmap.CompressFormat.JPEG,45,baos);if(bmp!=full)full.recycle();bmp.recycle();String encoded=Base64.encodeToString(baos.toByteArray(),Base64.NO_WRAP);JSONObject frame=new JSONObject().put("op","screen.frame").put("width",captureWidth).put("height",captureHeight).put("jpeg",encoded);screenClient.writeJson(frame);}catch(Exception ignored){}finally{if(image!=null)image.close();}},handler);
    ...
    void stopScreen(){screenClient=null;if(virtualDisplay!=null){virtualDisplay.release();virtualDisplay=null;}if(imageReader!=null){imageReader.close();imageReader=null;}}
    ...
    class Client extends Thread{
        ...
        public void run(){try{JSONObject hello=readJson();if(!"hello".equals(hello.optString("op"))){close();return;}writeJson(new JSONObject().put("ok",true).put("pairing_required",true).put("device_name",Build.MODEL));JSONObject pair=readJson();if(!pairingCode.equals(pair.optString("code"))){writeJson(new JSONObject().put("ok",false).put("error","Invalid pairing code"));close();return;}paired=true;writeJson(new JSONObject().put("ok",true).put("device_name",Build.MODEL));postStatus("● Paired with a desktop");while(paired){JSONObject r=readJson();handle(r);}}catch(Exception ignored){}finally{if(screenClient==this)stopScreen();close();}}
        void handle(JSONObject r)throws Exception{String op=r.optString("op");
        ...
        void mkdir(String path,String name)throws Exception{DocumentFile d=resolve(path);if(d==null||!d.isDirectory()||name.isEmpty()||name.contains("/")){writeJson(new JSONObject().put("ok",false));return;}writeJson(new JSONObject().put("ok",d.createDirectory(name)!=null));}
